...
import { OnlinePaymentCard } from "@/components/branch-settings/online-payment-card"
import { CashPaymentCard } from "@/components/branch-settings/cash-payment-card"
import { CardPaymentCard } from "@/components/branch-settings/card-payment-card"
import { useToast } from "@/hooks/use-toast"
import Link from "next/link"

export default function BranchPaymentMethodsPage() {
    ...
    if (success) {
      ...
      toast({
        ...
        description: language === 'fr'
          ? t.settingsBranch.paymentMethods.saved
          : t.settingsBranch.paymentMethods.saved,
        duration: 3000,
      })
    }
  ...
  // Payment method handlers - SW-78 FO-116
  const handleOnlinePaymentChange = (enabled: boolean) => {
    updateSettings({
      paymentSettings: {
        ...settings.paymentSettings,
        allowOnlinePayment: enabled,
        allowCashPayment: settings.paymentSettings?.allowCashPayment ?? false,
        allowCardPayment: settings.paymentSettings?.allowCardPayment ?? false,
          ...
          <div className="flex flex-1 flex-col px-2 sm:px-4 lg:px-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-8">
                  ...
                  <p className="text-muted-foreground mt-2 text-lg">
                    {language === 'fr'
                      ? 'Configurez les options de paiement pour votre succursale (WEB-SRM Québec)'
                      : 'Configure payment options for your branch (Quebec WEB-SRM)'}
                  </p>
                </div>
            ...
            <div className="flex-1 px-2 py-8 sm:px-4 lg:px-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <CardPaymentCard
                  allowCardPayment={settings.paymentSettings?.allowCardPayment ?? false}
                  onCardPaymentChange={handleCardPaymentChange}
                />
              </div>
            </div>
          </div>
