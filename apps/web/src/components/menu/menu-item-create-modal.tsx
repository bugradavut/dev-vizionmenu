...
// import { translations } from '@/lib/translations' // TODO: Use centralized translations
import type { 
  MenuItem, 
  MenuCategory, 
...
interface MenuItemCreateModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: CreateMenuItemRequest, photo?: File) => Promise<void>
...
const createMenuItemSchema = (language: string) => z.object({
  name: z.string()
...
type MenuItemFormData = z.infer<ReturnType<typeof createMenuItemSchema>>

export const MenuItemCreateModal: React.FC<MenuItemCreateModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  categories,
  ...
  // const t = translations[language] || translations.en // TODO: Implement centralized translations
  ...
  const form = useForm<MenuItemFormData>({
    resolver: zodResolver(createMenuItemSchema(language)),
    defaultValues: {
      name: item?.name || '',
  ...
  const handlePhotoSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Optimization completed (silent success)
  ...
  const handleSubmit = async (data: MenuItemFormData) => {
    ...
    try {
      ...
      await onSubmit(submitData, selectedPhoto || undefined)
      form.reset()
      setSelectedPhoto(null)
      setPhotoPreview(null)
      onClose()
  ...
  React.useEffect(() => {
    if (item && isOpen) {
      // Fix category_id population - use nested category.id if category_id is missing
      const categoryId = item.category_id || item.category?.id || ''
      
      console.log('Debug edit modal population:', {
        itemId: item.id,
        itemName: item.name,
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode 
              ? (language === 'fr' ? 'Modifier l\'article' : 'Edit Menu Item')
              : (language === 'fr' ? 'Nouvel article' : 'New Menu Item')
            }
          </DialogTitle>
          <DialogDescription>
            {isEditMode 
              ? (language === 'fr' 
                  ? 'Modifiez les détails de cet article de menu.'
                  : 'Edit the details of this menu item.')
              : (language === 'fr' 
                  ? 'Ajoutez un nouvel article à votre menu avec photo, prix et variantes.'
                  : 'Add a new item to your menu with photo, pricing, and variants.')
            }
          </DialogDescription>
        </DialogHeader>
                  ...
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base">
                        {language === 'fr' ? 'Article disponible' : 'Item Available'}
                      </FormLabel>
                      <FormDescription>
                        {language === 'fr' 
                          ? 'Les articles indisponibles sont masqués des clients'
            ...
            <DialogFooter>
              ...
              >
                {isSubmitting ? (
                  ...
                  language === 'fr' ? 'Mettre à jour' : 'Update Item'
                ) : (
                  language === 'fr' ? 'Créer l\'article' : 'Create Item'
                )}
              </Button>
            </DialogFooter>
          </form>
