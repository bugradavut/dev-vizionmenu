...
interface OrderNotificationSoundCardProps {
  orderSound: string
  onOrderSoundChange: (sound: string) => void
}

export function OrderNotificationSoundCard({
  orderSound,
  onOrderSoundChange
}: OrderNotificationSoundCardProps) {
  ...
  const { playSound } = useNotificationSound({
    soundUrl: getSoundPath(orderSound),
    enabled: true,
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <CardTitle className="text-base">{soundT.orderNotifications}</CardTitle>
            <p className="text-xs text-muted-foreground mt-1">
              {soundT.orderNotificationsDesc}
            </p>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="flex gap-2 items-center">
            <Select value={orderSound} onValueChange={onOrderSoundChange}>
