...
import { Truck, Clock, Check as CheckIcon } from "lucide-react"
...
// Types
type DeliveryHoursDay = "mon" | "tue" | "wed" | "thu" | "fri" | "sat" | "sun"

interface DeliveryHoursCopy {
  title: string
  subtitle: string
  statusClosed: string
  statusOpen: string
  ...
  openLabel: string
  closeLabel: string
  dayLabels: Record<DeliveryHoursDay, string>
  dayInitials: Record<DeliveryHoursDay, string>
}
...
interface DeliveryHoursCardProps {
  ...
  closeTime: string
  language: 'en' | 'fr'
  translations: DeliveryHoursCopy
  onDeliveryClosedToggle: (value: boolean) => void
  onWorkingDayToggle: (day: DeliveryHoursDay) => void
...
export function DeliveryHoursCard({
  ...
  language,
  translations,
  onDeliveryClosedToggle,
  onWorkingDayToggle,
  onOpenTimeChange,
  onCloseTimeChange,
}: DeliveryHoursCardProps) {
  const workingDayOrder: DeliveryHoursDay[] = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"]
    ...
    <Card className="h-full border border-blue-100 shadow-sm">
        <CardHeader className="pb-4">
          <div className="flex items-start justify-between gap-4">
            ...
            >
              {!deliveryClosed ? translations.statusOpen : translations.statusClosed}
              <Switch
                aria-label={translations.closedToggleAria}
                checked={!deliveryClosed}
                onCheckedChange={(checked) => onDeliveryClosedToggle(!checked)}
          ...
          <div className="flex flex-col gap-6 lg:flex-row">
            ...
            <div className="flex-1 space-y-3">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="delivery-hours-close" className="text-xs font-medium">
                    {translations.closeLabel}
                  </Label>
                  <div className="relative">
                    <Clock className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
                    <CustomTimePicker
                      id="delivery-hours-close"
                      value={closeTime}
