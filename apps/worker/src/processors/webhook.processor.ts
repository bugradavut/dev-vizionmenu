...
export class WebhookProcessor {
  async processJob(job: Job): Promise<JobResult> {
    try {
      switch (job.name) {
        case JOB_TYPES.PROCESS_STRIPE_WEBHOOK:
          return await this.processStripeWebhook(
            job.data as ProcessStripeWebhookJob,
          );
        case JOB_TYPES.PROCESS_THIRD_PARTY_WEBHOOK:
          return await this.processThirdPartyWebhook(
            job.data as ProcessThirdPartyWebhookJob,
          );
        default:
  ...
  private async processStripeWebhook(
    data: ProcessStripeWebhookJob,
  ): Promise<JobResult> {
    logger.info(`Processing Stripe webhook: ${data.event.type}`);
    ...
    try {
      ...
      switch (data.event.type) {
        ...
        default:
          logger.info(`Unhandled Stripe webhook type: ${data.event.type}`);
          return {
            success: true,
  ...
  private async processThirdPartyWebhook(
    data: ProcessThirdPartyWebhookJob,
  ): Promise<JobResult> {
    logger.info(`Processing ${data.provider} webhook`);
    ...
    try {
      ...
      logger.error(`Error processing ${data.provider} webhook:`, error);
      return {
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      };
  ...
  private async handlePaymentSucceeded(paymentIntent: any): Promise<JobResult> {
  ...
  private async handlePaymentFailed(paymentIntent: any): Promise<JobResult> {
  ...
  private async handleSubscriptionPayment(invoice: any): Promise<JobResult> {
  ...
  private async processUberEatsWebhook(
    payload: any,
    headers: Record<string, string>,
  ): Promise<JobResult> {
    logger.info("Processing Uber Eats webhook:", payload.event_type);
    ...
    switch (payload.event_type) {
      ...
      default:
        logger.info(`Unhandled Uber Eats webhook type: ${payload.event_type}`);
        return {
          success: true,
          message: "Webhook received but not processed",
        };
  ...
  private async processDoorDashWebhook(
    payload: any,
    headers: Record<string, string>,
  ): Promise<JobResult> {
    logger.info("Processing DoorDash webhook:", payload.event_type);
    ...
    switch (payload.event_type) {
      ...
      default:
        logger.info(`Unhandled DoorDash webhook type: ${payload.event_type}`);
        return {
          success: true,
          message: "Webhook received but not processed",
        };
  ...
  private async processGrubHubWebhook(
    payload: any,
    headers: Record<string, string>,
  ): Promise<JobResult> {
    logger.info("Processing GrubHub webhook:", payload.event_type);
  ...
  private async handleThirdPartyOrderPlaced(
    payload: any,
    provider: string,
  ): Promise<JobResult> {
    logger.info(`New ${provider} order placed:`, payload.order_id);
  ...
  private async handleThirdPartyOrderUpdated(
    payload: any,
    provider: string,
  ): Promise<JobResult> {
    logger.info(`${provider} order updated:`, payload.order_id);
  ...
  private async handleThirdPartyOrderCanceled(
    payload: any,
    provider: string,
  ): Promise<JobResult> {
    logger.info(`${provider} order canceled:`, payload.order_id);
  ...
  private verifyStripeSignature(event: any, signature: string): boolean {
