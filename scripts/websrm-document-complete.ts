/**
 * WEB-SRM DEV - /document Complete Test (Iterative)
 *
...
// DEV Headers + Document-specific headers
const DEV_HEADERS = {
  ...
  VERSI: '0.1.0',
  CODAUTORI: 'D8T8-W8W8',
  // Document-specific (from error messages)
...
// Create dummy Base64 document (simple text)
const createDummyDocument = () => {
  const content = JSON.stringify({
    type: 'test-document',
    content: 'Test document content for WEB-SRM validation',
...
const PAYLOAD_ITERATIONS = [
  // Iteration 1: Empty
  {},
  // Iteration 2: Add reqDoc
  {
    reqDoc: {},
  },
  // Iteration 3: Add typDoc
  {
    reqDoc: {
      typDoc: 'ATTEST',
    },
  },
  // Iteration 4: Add doc (Base64)
  {
    reqDoc: {
      typDoc: 'ATTEST',
  ...
  // Iteration 5: Try different typDoc if needed
  {
    reqDoc: {
      typDoc: 'FACTURE',
      doc: createDummyDocument(),
...
async function testIteration(
  ...
  // Save request (truncate doc for readability)
  const displayPayload = JSON.parse(JSON.stringify(payload));
  if (displayPayload.reqDoc?.doc) {
    displayPayload.reqDoc.doc = displayPayload.reqDoc.doc.substring(0, 50) + '... (truncated)';
  }
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║  WEB-SRM DEV - /document Complete Test (Iterative)            ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');

  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const outputDir = path.join('tmp', 'logs', `dev-document-complete-${timestamp}`);
  ...
  const displayPayload = JSON.parse(JSON.stringify(finalResult.payload));
  if (displayPayload.reqDoc?.doc) {
    displayPayload.reqDoc.doc = '[BASE64_ENCODED_DOCUMENT]';
  }

  const summary = `# /document Complete Test Summary (Iterative)
...
**Note**: \`doc\` field contains Base64-encoded document content.
...
${
  finalResult.errors.length > 0
    ? `| # | ID | Code | Message |
|---|----|----|---------|
${finalResult.errors.map((e, i) => `| ${i + 1} | ${e.id} | ${e.codRetour || 'N/A'} | ${e.mess?.substring(0, 80)} |`).join('\n')}`
    : '✅ No errors'
}
...
${
  finalResult.errors.length > 0
    ...
    : 'No errors - document structure is valid.'
}`
